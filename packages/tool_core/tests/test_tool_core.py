...
class WebBuiltinTests(unittest.TestCase):
    def test_web_search_tool_shape(self):
        t = web_search_tool()
        self.assertEqual(t.name, "web_search")
        self.assertEqual(t.risk, "safe")
        self.assertIn("query", t.parameters)

    def test_web_search_missing_env_returns_false(self):
        real_import = builtins.__import__
        def fake_import(name, *args, **kwargs):
            if name == "env_core":
                class E:
            ...
            return real_import(name, *args, **kwargs)
        with patch("builtins.__import__", side_effect=fake_import):
            res = web_search_tool().call({"query": "x"})
            self.assertFalse(res.ok)

    def test_web_search_default_and_clamped_num_results(self):
        calls = []
        ...
        def fake_import(name, *args, **kwargs):
            if name == "env_core":
                class E:
                    ...
                    @staticmethod
                    def require_env(_):
                        return E.calls.pop(0)
                return E
            return real_import(name, *args, **kwargs)
        with patch("builtins.__import__", side_effect=fake_import), patch("tool_core.builtins.urlopen", side_effect=fake_open):
            web_search_tool().call({"query": "a"})
            web_search_tool().call({"query": "a", "num_results": 99})
        self.assertIn("num=5", calls[0])
        self.assertIn("num=10", calls[1])

    def test_web_search_formats_results(self):
        payload = b'{"items":[{"title":"T","link":"L","snippet":"S"}]}'
        class R:
            def read(self):
                return payload
        real_import = builtins.__import__
        def fake_import(name, *args, **kwargs):
            if name == "env_core":
                ...
                return E
            return real_import(name, *args, **kwargs)
        with patch("builtins.__import__", side_effect=fake_import), patch("tool_core.builtins.urlopen", return_value=R()):
            res = web_search_tool().call({"query": "x"})
            self.assertTrue(res.ok)
            self.assertIn("Title: T", res.output)
            self.assertEqual(res.metadata["tool_name"], "web_search")
    ...
    def test_web_search_request_error(self):
        ...
        def fake_import(name, *args, **kwargs):
            if name == "env_core":
                ...
                return E
            return real_import(name, *args, **kwargs)
        with patch("builtins.__import__", side_effect=fake_import), patch("tool_core.builtins.urlopen", side_effect=RuntimeError("boom")):
            res = web_search_tool().call({"query": "x"})
            self.assertFalse(res.ok)
            self.assertEqual(res.metadata["tool_name"], "web_search")

    def test_fetch_url_tool_shape(self):
        t = fetch_url_tool()
        self.assertEqual(t.name, "fetch_url")
        self.assertEqual(t.risk, "safe")

    def test_fetch_url_user_agent_and_default_max_chars(self):
        seen = {}
        class R:
            def read(self):
                return ("a" * 20000).encode()
        def fake_open(req, timeout=0):
            seen["ua"] = req.headers.get("User-agent")
            return R()
        with patch("tool_core.builtins.urlopen", side_effect=fake_open):
            res = fetch_url_tool().call({"url": "https://example.com"})
            self.assertEqual(len(res.output), 12000)
            self.assertEqual(res.metadata["tool_name"], "fetch_url")
        self.assertEqual(seen["ua"], "AI-Modules/1.0")

    def test_fetch_url_clamped_max_chars(self):
        class R:
            def read(self):
                return ("b" * 60000).encode()
        with patch("tool_core.builtins.urlopen", return_value=R()):
            short = fetch_url_tool().call({"url": "https://x", "max_chars": 1})
            long = fetch_url_tool().call({"url": "https://x", "max_chars": 999999})
            self.assertEqual(len(short.output), 1000)
            self.assertEqual(len(long.output), 50000)

    def test_fetch_url_error_returns_false(self):
        with patch("tool_core.builtins.urlopen", side_effect=RuntimeError("boom")):
            res = fetch_url_tool().call({"url": "https://x"})
            self.assertFalse(res.ok)
            self.assertEqual(res.metadata["tool_name"], "fetch_url")
