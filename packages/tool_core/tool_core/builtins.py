...
def web_search_tool() -> Tool:
    def handler(query: str, num_results: int = 5) -> ToolResult:
        started = time.time()
        try:
            from env_core import require_env

            key = require_env("GOOGLE_SEARCH_API_KEY")
            cx = require_env("GOOGLE_SEARCH_ENGINE_ID")
            num = max(1, min(int(num_results), 10))
            url = "https://www.googleapis.com/customsearch/v1?" + urlencode({"key": key, "cx": cx, "q": query, "num": num})
            ...
            for item in items:
                ...
                lines.append("")
            ended = time.time()
            return ToolResult(ok=True, output="\n".join(lines).strip(), metadata={"tool_name": "web_search", "args": {"query": query, "num_results": num}, "duration_ms": int((ended-started)*1000)})
        except Exception as exc:
            ended = time.time()
            return ToolResult(ok=False, error=str(exc), metadata={"tool_name": "web_search", "args": {"query": query, "num_results": num_results}, "duration_ms": int((ended-started)*1000)})

    return Tool(name="web_search", description="Search the web using Google Custom Search API", parameters={"query": {"type": "str", "required": True}, "num_results": {"type": "int", "required": False}}, handler=handler, risk="safe")


def fetch_url_tool() -> Tool:
    def handler(url: str, max_chars: int = 12000) -> ToolResult:
        started = time.time()
        try:
            cap = max(1000, min(int(max_chars), 50000))
            req = Request(url, headers={"User-Agent": "AI-Modules/1.0"})
            text = urlopen(req, timeout=20).read().decode("utf-8", errors="replace")
            ended = time.time()
            return ToolResult(ok=True, output=text[:cap], metadata={"tool_name": "fetch_url", "args": {"url": url, "max_chars": cap}, "duration_ms": int((ended-started)*1000)})
        except Exception as exc:
            ended = time.time()
            return ToolResult(ok=False, error=str(exc), metadata={"tool_name": "fetch_url", "args": {"url": url, "max_chars": max_chars}, "duration_ms": int((ended-started)*1000)})

    return Tool(name="fetch_url", description="Fetch URL content", parameters={"url": {"type": "str", "required": True}, "max_chars": {"type": "int", "required": False}}, handler=handler, risk="safe")
...
# ... remaining existing helpers unchanged
    ...
    def handler(path: str, search: str, replace: str, dry_run: bool = True, match_mode: str = "exact") -> ToolResult:
        ...
        try:
            if match_mode == "exact":
                ...
                if matches == 0: return ToolResult(ok=False, error="search block not found", metadata={"matches": 0})
                if matches > 1: return ToolResult(ok=False, error="multiple search block matches found", metadata={"matches": matches})
                patched = original.replace(search, replace, 1)
            elif match_mode == "line_endings":
                nl = _dominant_line_ending(original); src_n = _normalize_line_endings(original); search_n = _normalize_line_endings(search); replace_n = _normalize_line_endings(replace)
                matches = src_n.count(search_n)
                if matches == 0: return ToolResult(ok=False, error="search block not found", metadata={"matches": 0})
                if matches > 1: return ToolResult(ok=False, error="multiple search block matches found", metadata={"matches": matches})
                patched = src_n.replace(search_n, replace_n, 1).replace("\n", nl)
            else:
                found = list(_whitespace_regex(search).finditer(original)); matches = len(found)
                if matches == 0: return ToolResult(ok=False, error="search block not found", metadata={"matches": 0})
                if matches > 1: return ToolResult(ok=False, error="multiple search block matches found", metadata={"matches": matches})
                m = found[0]; patched = original[:m.start()] + replace + original[m.end():]
        except Exception as exc:
            return ToolResult(ok=False, error=f"patch failure: {exc}")
        if dry_run: return ToolResult(ok=True, output="Patch dry run succeeded", metadata={"matches": 1, "dry_run": True})
        try:
