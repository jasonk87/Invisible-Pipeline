...
def run_pipeline(
    ...
    generator = generate_fn or generate_text

    answer = generator(prompt=generation_prompt(task), model=model, timeout=timeout)
    rounds_used = 1

    for _ in range(max_rounds - 1):
        result = generator(prompt=evaluation_prompt(task, answer, facts), model=model, timeout=timeout)
        rounds_used += 1
