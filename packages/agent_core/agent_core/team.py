...
class AgentTeam:
    def __init__(
        ...
        if team_mode not in ("round_robin", "orchestrator"):
            raise ValueError("team_mode must be 'round_robin' or 'orchestrator'")
        if team_mode == "orchestrator":
            if orchestrator is None:
                raise ValueError("orchestrator is required for orchestrator mode")
            if not hasattr(orchestrator, "generate") or not hasattr(orchestrator, "name"):
                raise ValueError("orchestrator must have name and generate")
