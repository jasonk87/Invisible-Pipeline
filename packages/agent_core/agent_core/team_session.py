...
class TeamSession:
        ...
        if getattr(result, "metadata", None):
            ...
            for ev in md.get("events", []):
                ...
                if isinstance(details, dict) and isinstance(details.get("metadata"), dict):
                    self.tool_traces.append({"agent": agent_name, **details["metadata"]})

    def _result(self, text, completed, stop_reason) -> TeamResult:
        self.completed = completed
        self.stop_reason = stop_reason
        return TeamResult(text=text, completed=completed, stop_reason=stop_reason, transcript=self.transcript, events=self.events, metadata={"activity_log": self.activity_log, "session_id": self.session_id, "llm_traces": self.llm_traces, "tool_traces": self.tool_traces})
    ...
    def _run_round_robin(self) -> TeamResult:
        ...
        for _ in range(self.team.max_turns):
            ...
            if getattr(agent, "llm", None) is not None and hasattr(agent.llm, "clear_traces"):
                agent.llm.clear_traces()
            result = agent.generate(self._build_prompt(agent))
            if hasattr(agent, "collect_llm_traces"):
                for tr in agent.collect_llm_traces():
                    self.llm_traces.append({"agent": agent.name, "trace": tr})
    ...
    def _run_orchestrator(self) -> TeamResult:
        for _ in range(self.team.max_turns):
            ...
            if getattr(worker, "llm", None) is not None and hasattr(worker.llm, "clear_traces"):
                worker.llm.clear_traces()
            result = worker.generate(self._build_worker_prompt(worker, instruction))
            if hasattr(worker, "collect_llm_traces"):
                for tr in worker.collect_llm_traces():
                    self.llm_traces.append({"agent": worker.name, "trace": tr})
