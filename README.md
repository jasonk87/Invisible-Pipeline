...
In orchestrator mode, the orchestrator must return strict JSON to select the next speaker (or finish). Completion policies are not applied inside orchestrator mode in Phase 1; completion comes from orchestrator `{"done": true, "final_answer": "..."}`. Round-robin remains the default mode.
